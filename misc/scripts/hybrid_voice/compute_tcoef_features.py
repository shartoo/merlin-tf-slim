...
def findHybridParamRichContexts(file_id_list, feat_dict, vfloor, data_dir, tcoef_dir, lab_dir, sil_identifier,
                                ignoreSilence=True):
    ...
    for file_index in range(len(file_id_list)):
        ...
        outf.write(
            'EST_File Track\nDataType ascii\nNumFrames {0}\nNumChannels {1}\nNumAuxChannels 0\nfile_type 14\nEST_Header_End\n'.format(
                len(hybridInfo[1]), len(features[0]) * 2))
        ...
        for x in range(len(hybridInfo[1])):
            ...
            if sil_identifier in hybridInfo[3][x]:
                ...
                tempContext = processHybridInfo.ContextInfo(float(hybridInfo[0][x]), hybridInfo[1][x], hybridInfo[2][x],
                                                            hybridInfo[3][x], temp[x])
...
if __name__ == '__main__':
    ...
    tempFeats = findHybridParamRichContexts(train_id_list, feat_dict, vfloor, data_dir, tcoef_train_dir, in_lab_dir,
                                            sil_identifier)

    print('computing tcoef features for test data...')
    tempFeats = findHybridParamRichContexts(test_id_list, feat_dict, vfloor, data_dir, tcoef_test_dir, in_lab_dir,
                                            sil_identifier)
