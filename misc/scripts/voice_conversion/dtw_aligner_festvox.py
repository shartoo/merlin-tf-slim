...
if len(sys.argv) != 6:
    print(
        "Usage: python dtw_aligner_festvox.py <path_to_tools_dir> <path_to_src_feat_dir> <path_to_tgt_feat_dir> <path_to_src_aligned_feat_dir> <bap_dim>")
    sys.exit(1)
...
def process(filename):
    ...
    os.path.join(sptk, "x2x"), src_mgc_file, mgc_dim, os.path.join(temp_dir, file_id + "_src_ascii.mgc"))
    x2x_cmd2 = "%s +fa %s | xargs -n%d > %s" % (
    os.path.join(sptk, "x2x"), tgt_mgc_file, mgc_dim, os.path.join(temp_dir, file_id + "_tgt_ascii.mgc"))
    ...
    ### align features
    aligner.align_src_feats(os.path.join(src_mgc_dir, file_id + ".mgc"),
                            os.path.join(src_aligned_mgc_dir, file_id + ".mgc"), mgc_dim, dtw_path_dict)
    aligner.align_src_feats(os.path.join(src_bap_dir, file_id + ".bap"),
                            os.path.join(src_aligned_bap_dir, file_id + ".bap"), bap_dim, dtw_path_dict)
    aligner.align_src_feats(os.path.join(src_lf0_dir, file_id + ".lf0"),
