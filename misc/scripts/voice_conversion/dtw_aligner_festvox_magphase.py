...
if len(sys.argv) != 5:
    print(
        "Usage: python dtw_aligner_festvox_magphase.py <path_to_tools_dir> <path_to_src_feat_dir> <path_to_tgt_feat_dir> <path_to_src_aligned_feat_dir>")
    sys.exit(1)
...
def process(filename):
    ...
    x2x_cmd1 = "%s +fa %s | xargs -n%d > %s" % (
    os.path.join(sptk, "x2x"), src_mag_file, mag_dim, os.path.join(temp_dir, file_id + "_src_ascii.mag"))
    x2x_cmd2 = "%s +fa %s | xargs -n%d > %s" % (
    os.path.join(sptk, "x2x"), tgt_mag_file, mag_dim, os.path.join(temp_dir, file_id + "_tgt_ascii.mag"))
    ...
    os.path.join(festvox, "src/general/phonealign"), \
    ...
    ### align features
    aligner.align_src_feats(os.path.join(src_feat_dir, file_id + ".mag"),
                            os.path.join(src_aligned_feat_dir, file_id + ".mag"), mag_dim, dtw_path_dict)
    aligner.align_src_feats(os.path.join(src_feat_dir, file_id + ".real"),
                            os.path.join(src_aligned_feat_dir, file_id + ".real"), real_dim, dtw_path_dict)
    aligner.align_src_feats(os.path.join(src_feat_dir, file_id + ".imag"),
                            os.path.join(src_aligned_feat_dir, file_id + ".imag"), imag_dim, dtw_path_dict)
    aligner.align_src_feats(os.path.join(src_feat_dir, file_id + ".lf0"),
