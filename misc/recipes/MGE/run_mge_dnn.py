...
def train_DNN(train_xy_file_list, valid_xy_file_list, \
    ...
    while (epoch < training_epochs):
        ...
        while (not train_data_reader.is_finish()):
            ...
            logger.debug(
                'this partition: %d frames (divided into %d batches )' % (train_set_x.shape[0], n_train_batches))

            this_train_error = dnn_model.finetune((train_set_x, train_set_y), current_frame_number, current_finetune_lr,
                                                  current_momentum, mean_matrix, std_matrix)
        ...
        while (not valid_data_reader.is_finish()):
            ...
            this_valid_loss = dnn_model.parameter_prediction_trajectory(valid_set_x, valid_set_y, mean_matrix,
                                                                        std_matrix)
...
def main_function(cfg):
    ...
    if cfg.GENWAV:
        ...
        generate_wav(gen_dir, gen_file_id_list[cfg.valid_file_number:cfg.valid_file_number + cfg.test_file_number],
                     cfg)  # generated speech
