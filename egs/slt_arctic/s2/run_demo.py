...
if __name__ == '__main__':
    ...
    d_mp_opts['l_pf_type'] = ['no', 'magphase',
                              'merlin']  # List containing the postfilters to apply during waveform generation.
    ...
    # PROCESS:===================================================================================================
    # Pre setup:-------------------------------------------------------------------------------
    exper_name = 'slt_arctic_magphase_%s_mag_dim_%s_phase_dim_%d_const_rate_%d' % (
    exper_type, d_mp_opts['mag_dim'], d_mp_opts['phase_dim'], d_mp_opts['b_const_rate'])
    ...
    if b_download_data:
        ...
        call(['wget', 'http://felipeespic.com/depot/databases/merlin_demos/slt_arctic_%s_data.zip' % exper_type, '-O',
              data_zip_file])
    ...
    if b_config_merlin:
        ...
        copy2(join(this_dir, 'conf_base', 'logging_config.conf'),
    ...
    if b_feat_extr:
        ...
        if b_feat_ext_multiproc:
            lu.run_multithreaded(feat_extraction, join(exper_path, 'acoustic_model', 'data', 'wav'), l_file_tokns,
                                 acoustic_feats_path, d_mp_opts)
        else:
            for file_name_token in l_file_tokns:
                feat_extraction(join(exper_path, 'acoustic_model', 'data', 'wav'), file_name_token, acoustic_feats_path,
                                d_mp_opts)
