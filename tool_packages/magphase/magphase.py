...
# From (after) 'analysis_with_del_comp':
# new: returns voi/unv decision.
def analysis_with_del_comp_from_est_file(v_in_sig, est_file, fs, nFFT=None, win_func=np.hanning, b_ph_unv_zero=False,
                                         nwin_per_pitch_period=0.5):
    ...
    m_sp, m_ph, v_shift, m_frms, m_fft = analysis_with_del_comp_from_pm(v_in_sig, v_pm_smpls, nFFT, win_func=win_func,
                                                                        nwin_per_pitch_period=nwin_per_pitch_period)
...
# f0_type: 'f0', 'lf0'
def analysis_with_del_comp__ph_enc__f0_norm__from_files(wav_file, est_file, nFFT, mvf, f0_type='f0',
                                                        b_ph_unv_zero=False, win_func=np.hanning):
    m_spmgc, m_phs_mgc, m_phc_mgc, v_shift, v_voi, fs = analysis_with_del_comp_and_ph_encoding_from_files(wav_file,
                                                                                                          est_file,
                                                                                                          nFFT, mvf,
                                                                                                          b_ph_unv_zero=b_ph_unv_zero,
                                                                                                          win_func=win_func)
...
def analysis_with_del_comp__ph_enc__f0_norm__from_files_raw(wav_file, est_file, nFFT=None, win_func=np.hanning,
                                                            nwin_per_pitch_period=0.5):
    '''
    ...
    if (fs != 48000) or (fs != 16000):
        raise ValueError(
            'MagPhase works only at 16kHz and 48kHz sample rates, for now. The wavefile\'s sample rate is %d (Hz).\nConsider resampling the data beforehand if wanted.' % (
                fs))

    # Analysis:----------------------------------------------------------------
    m_sp_dummy, m_ph_dummy, v_shift, v_voi, m_frms, m_fft = analysis_with_del_comp_from_est_file(v_in_sig, est_file, fs,
                                                                                                 nFFT=nFFT,
                                                                                                 win_func=win_func,
                                                                                                 nwin_per_pitch_period=nwin_per_pitch_period)
...
def analysis_with_del_comp__ph_enc__f0_norm__from_files2(wav_file, est_file, mvf, nFFT=None, f0_type='f0',
                                                         win_func=np.hanning, mag_mel_nbins=60, cmplx_ph_mel_nbins=45):
    m_mag, m_real, m_imag, v_shift, v_voi, m_frms, fs = analysis_with_del_comp__ph_enc__f0_norm__from_files_raw(
        wav_file, est_file, nFFT=nFFT, win_func=win_func)
...
# After 'analysis_with_del_comp_and_ph_encoding'
# new: returns voi/unv decision.
def analysis_with_del_comp_and_ph_encoding_from_files(wav_file, est_file, nFFT, mvf, b_ph_unv_zero=False,
                                                      win_func=np.hanning):
    # Read wav file:
    v_in_sig, fs = sf.read(wav_file)

    m_sp, m_ph, v_shift, v_voi, m_frms = analysis_with_del_comp_from_est_file(v_in_sig, est_file, nFFT, fs,
                                                                              b_ph_unv_zero=b_ph_unv_zero,
                                                                              win_func=win_func)
...
def synthesis_with_del_comp__ph_enc__from_f0(m_spmgc, m_phs, m_phc, v_f0, nFFT, fs, mvf, ph_hf_gen, v_voi='estim'):
    ...
    v_syn_sig = synthesis_with_del_comp_and_ph_encoding(m_spmgc, m_phs, m_phc, v_shift, nFFT, fs, mvf, ph_hf_gen,
                                                        v_voi=v_voi)
...
def synthesis_from_compressed_type1_old_with_griffin_lim(m_mag_mel_log, m_real_mel, m_imag_mel, v_lf0, fs, fft_len=None,
                                                         hf_slope_coeff=1.0, b_voi_ap_win=True, b_fbank_mel=False,
                                                         const_rate_ms=-1.0,
                                                         per_phase_type='magphase', griff_lim_type=None,
                                                         griff_lim_init='magphase'):
    '''
    ...
    if const_rate_ms > 0.0:
        ...
        v_shift, v_frm_locs_smpls = get_shifts_and_frm_locs_from_const_shifts(v_shift, const_rate_ms, fs,
                                                                              interp_type=interp_type)
        m_mag = interp_from_const_to_variable_rate(m_mag, v_frm_locs_smpls, const_rate_ms, fs, interp_type=interp_type)
        m_real = interp_from_const_to_variable_rate(m_real, v_frm_locs_smpls, const_rate_ms, fs,
                                                    interp_type=interp_type)
        m_imag = interp_from_const_to_variable_rate(m_imag, v_frm_locs_smpls, const_rate_ms, fs,
                                                    interp_type=interp_type)
        v_voi = interp_from_const_to_variable_rate(v_f0 > 0.0, v_frm_locs_smpls, const_rate_ms, fs,
                                                   interp_type=interp_type) > 0.5
        v_f0 = shift_to_f0(v_shift, v_voi, fs, out='f0', b_smooth=False)
        nfrms = v_shift.size
    ...
    m_ap_mask[v_voi, :] = la.spectral_crossfade(m_zeros[v_voi, :], m_ap_mask[v_voi, :], crsf_cf, crsf_bw, fs,
                                                freq_scale='hz')
    ...
    m_det_mask[~v_voi, :] = 0
    m_det_mask[v_voi, :] = la.spectral_crossfade(m_det_mask[v_voi, :], m_zeros[v_voi, :], crsf_cf, crsf_bw, fs,
                                                 freq_scale='hz')
...
def synthesis_from_compressed(m_mag_mel_log, m_real_mel, m_imag_mel, v_lf0, fs, fft_len=None, b_voi_ap_win=True,
    ...
    if b_const_rate:
        ...
        m_imag = interp_from_const_to_variable_rate(m_imag, v_frm_locs_smpls, const_rate_ms, fs,
                                                    interp_type=interp_type)
        v_voi = interp_from_const_to_variable_rate(v_voi, v_frm_locs_smpls, const_rate_ms, fs,
                                                   interp_type=interp_type) > 0.5
        v_f0 = shift_to_f0(v_shift, v_voi, fs, out='f0', b_smooth=False)
        nfrms = v_shift.size
    ...
    for nxf in range(nfrms):
        v_win = la.gen_centr_win(v_shift_ext[nxf] + v_shift_ext[nxf + 1], v_shift_ext[nxf + 2] + v_shift_ext[nxf + 3],
                                 frmlen, win_func=raised_hanning, b_fill_w_bound_val=True)
...
def synthesis_from_compressed_type1_with_phase_comp(m_mag_mel_log, m_real_mel, m_imag_mel, v_lf0, fs, fft_len=None,
                                                    b_voi_ap_win=True, b_fbank_mel=False, const_rate_ms=-1.0,
    ...
    if per_phase_type == 'magphase':
        ...
        if False:
            ...
            nx = 146;
            figure();
            plot(np.angle(m_per_cmplx_ph[nx, :]));
            grid()
            nx = 146;
            figure();
            plot(np.angle(m_per_cmplx_ph[nx, :]));
    ...
    if False:
        ...
        nx = 73;
        figure();
        plot(np.angle(m_syn_cmplx[nx, :]), '.-');
        grid()
        nx = 73;
        figure();
        plot(m_syn_mag_db[nx, :], '.-');
        grid()
...
def synthesis_from_compressed_type1(m_mag_mel_log, m_real_mel, m_imag_mel, v_lf0, fs, fft_len=None,
                                    b_voi_ap_win=True, b_fbank_mel=False, b_const_rate=False,
                                    per_phase_type='magphase'):
    '''
    b_fbank_mel: If True, Mel compression done by the filter bank approach. Otherwise, it uses sptk mcep related funcs.
    per_phase_type: 'magphase', 'min_phase', or 'linear'
...
def synthesis_from_compressed_type2(m_mag_mel_log, m_real_mel, m_imag_mel, v_lf0, fs, fft_len=None, hf_slope_coeff=1.0,
    ...
    m_zeros = np.zeros((nfrms, fft_len_half))
    m_ap_mag_smth[v_voi, :] = la.spectral_crossfade(m_zeros[v_voi, :], m_ap_mag_smth[v_voi, :], crsf_cf, crsf_bw, fs,
                                                    freq_scale='hz')
...
# if v_voi[n] > 0, frame is voiced. If v_voi[n] == 0, frame is unvoiced.
# If v_voy=='estim', the mask is estimated from phase data.
def synthesis_with_del_comp_and_ph_encoding4(m_spmgc, m_phs_mgc, m_phc_mgc, v_shift, nFFT, fs, mvf, v_voi,
                                             b_medfilt=False, win_func=None):
    # Protection:
    v_shift = v_shift.astype(int)
    ...
    for i in range(nfrms):
        ...
        if v_voi_mask[i] == 1:  # voiced case
            ...
            # Phase:--------------------------------------------
            v_ph = \
                la.spectral_crossfade(m_ph_deter[None, i, :], m_voi_noise_ph[None, i, :], cut_off, bw, fs,
                                      freq_scale='hz')[
                    0]
            ...
            v_phsA = \
                la.spectral_crossfade(v_ph_deters[None, :], v_voi_noise_phs[None, :], 5000, 2000, fs, freq_scale='hz')[
                    0]
            v_phcA = \
                la.spectral_crossfade(v_ph_deterc[None, :], v_voi_noise_phc[None, :], 5000, 2000, fs, freq_scale='hz')[
                    0]
...
# if v_voi[n] > 0, frame is voiced. If v_voi[n] == 0, frame is unvoiced.
# If v_voy=='estim', the mask is estimated from phase data.
def synthesis_with_del_comp_and_ph_encoding3(m_spmgc, m_phs_mgc, m_phc_mgc, v_shift, nFFT, fs, mvf, v_voi,
                                             b_medfilt=False):
    # Protection:
    v_shift = v_shift.astype(int)
...
# if v_voi[n] > 0, frame is voiced. If v_voi[n] == 0, frame is unvoiced.
# If v_voy=='estim', the mask is estimated from phase data.
def synthesis_with_del_comp_and_ph_encoding(m_spmgc, m_phs_mgc, m_phc_mgc, v_shift, nFFT, fs, mvf, ph_hf_gen="rand",
                                            v_voi='estim', win_func=np.hanning, win_flat_to_len=0.3):
    # MGC to SP:
...
def to_constant_rate(m_data, targ_shift_ms, v_shift, fs, interp_kind='linear'):
    ...
    v_targ_cntr_nxs = np.arange(targ_shift_smpls, in_est_sig_len,
                                targ_shift_smpls)  # checkear que el codigo DNN indexe los frames asi tamnbien!
...
def get_num_of_frms_per_state(v_shift, lab_state_align_file, fs, b_prevent_zeros=False, n_states_x_phone=5,
                              nfrms_tolerance=6):
    # Read lab file:
...
def interp_from_variable_to_const_frm_rate(m_data, v_pm_smpls, const_rate_ms, fs, interp_type='linear'):
    ...
    if v_pm_smpls[0] > 0:  # Protection
        f_interp = interpolate.interp1d(np.r_[0, v_pm_smpls], np.vstack((m_data[0, :], m_data)), axis=0,
                                        kind=interp_type)
    else:
...
def post_filter_backup_old(m_mag_mel_log):
    ...
    if ncoeffs != 60:
        warnings.warn(
            'The postfilter has been only tested with 60 dimensional mag data. If you use another dimension, the result may be suboptimal.')
...
def post_filter(m_mag_mel_log, fs, av_len_at_zero=None, av_len_at_nyq=None, boost_at_zero=None, boost_at_nyq=None):
    ...
    if mag_dim != 60:
        warnings.warn(
            'Post-filter: It has been only tested with 60 dimensional mag data. If you use another dimension, the result may be suboptimal.')
    ...
    for nxf in range(nfrms):
        ...
        if False:
            from libplot import lp;
            lp.figure();
            lp.plot(v_mag_mel_log);
            lp.plot(v_ave);
            lp.plot(v_mag_mel_log_norm);
            lp.grid();
            lp.show()
...
def format_for_modelling(m_mag, m_real, m_imag, v_f0, fs, mag_dim=60, phase_dim=45, b_mag_fbank_mel=False,
                         alpha_phase=None):
    '''
    b_fbank_mel: If True, Mel compression done by the filter bank approach. Otherwise, it uses sptk mcep related funcs.
    '''
...
def format_for_modelling_phase_comp(m_mag, m_real, m_imag, v_f0, fs, mag_dim=60, phase_dim=10, b_mag_fbank_mel=False):
    ...
    if False:
        ...
        m_ratio_rec = np.hstack(
            (m_ratio_shrt_rec, m_ratio_shrt_rec[:, -1][:, None] + np.zeros((nfrms, fft_len_half - max_bin_ph))))
        ...
        m_real_rec = np.hstack(
            (m_real_shrt_rec, m_real_shrt_rec[:, -1][:, None] + np.zeros((nfrms, fft_len_half - max_bin_ph))))
        m_imag_rec = np.hstack(
            (m_imag_shrt_rec, m_imag_shrt_rec[:, -1][:, None] + np.zeros((nfrms, fft_len_half - max_bin_ph))))
        ...
        nx = 111;
        figure();
        plot(m_cmplx_ph_shrt);
        plot(np.abs(m_cmplx_ph_shrt));
        grid()
        ...
        plot(m_real_mel[nx, :], '.-');
        plot(m_ratio_mel[nx, :], '.-');
        plot(m_ratio_mel_from_params[nx, :], '.-');
        grid()
    ...
    if False:
        ...
        plot(v_ratio);
        plot(np.arctan(v_ratio));
        plot(np.angle(m_real[nx, :] + m_imag[nx, :] * 1j));
        grid()
        ...
        plot(m_real[nx, :]);
        plot(m_real_rec[nx, :]);
        plot(m_real_rec_max[nx, :]);
        grid()
        nx = 73;
        figure();
        plot(m_imag[nx, :]);
        plot(m_imag_rec[nx, :]);
        plot(m_imag_rec_max[nx, :]);
        grid()
        nx = 73;
        figure();
        plot(m_ph[nx, :], '.-');
        plot(np.angle(m_real_rec[nx, :] + m_imag_rec[nx, :] * 1j), '.-');
        plot(np.angle(m_real_rec_max[nx, :] + m_imag_rec_max[nx, :] * 1j), '.-');
        grid()
    ...
    if False:
        ...
        figure();
        plot(m_mag_log[252:255, :].T);
        plot(m_mag_log_rec[252:255, :].T);
        grid()
...
def format_for_modelling_old(m_mag, m_real, m_imag, v_f0, fs, mag_dim=60, phase_dim=45, b_fbank_mel=False):
    ...
    if False:
        ...
        nx = 171;
        lp.figure();
        lp.plot(np.log(m_mag[nx, :]));
        lp.plot(m_mag_log_rec[nx, :]);
        lp.plot(m_mag_log_rec_debug[nx, :]);
        lp.grid()
        nx = 20;
        lp.figure();
        lp.plot(np.log(m_mag[nx, :]));
    ...
    if False:
        ...
        lp.plot(m_mag[nx, :]);
        lp.plot(m_mag_mel_log[nx, :]);
        lp.plot(m_mag_mel_log_debug[nx, :]);
        lp.grid()
...
def analysis_lossless_type2(wav_file, fft_len=None, out_dir=None):
    ...
    # Phase analysis:------------------------------------------------------------------------
    m_fft_phase, v_shift_phase, v_gain = analysis_with_del_comp_from_pm_type2(v_sig, fs, v_pm_smpls, v_voi,
                                                                              fft_len=fft_len)
    ...
    if False:
        ...
        nx = 201;
        lp.figure();
        lp.plot(m_mag_log[nx, :]);
        lp.plot(m_mag_env_log[nx, :]);
        lp.plot(0.0 + np.log(m_mag[nx, :]));
        lp.grid()
        nx = 101;
        lp.figure();
        lp.plot(m_real[nx, :]);
...
def analysis_compressed_type1(wav_file, fft_len=None, out_dir=None, mag_dim=60, phase_dim=45, const_rate_ms=-1.0):
    ...
    if const_rate_ms > 0.0:
        ...
        v_f0 = interp_from_variable_to_const_frm_rate(np.r_[v_f0[v_voi][0], v_f0[v_voi], v_f0[v_voi][-1]],
                                                      np.r_[0, v_pm_smpls[v_voi], v_pm_smpls[-1]], const_rate_ms, fs,
                                                      interp_type=interp_type).squeeze()
        v_voi = interp_from_variable_to_const_frm_rate(v_voi, v_pm_smpls, const_rate_ms, fs,
                                                       interp_type=interp_type) > 0.5
        v_f0 *= v_voi  # Double check this. At the beginning of voiced segments.

    # Formatting for Acoustic Modelling:
    m_mag_mel_log, m_real_mel, m_imag_mel, v_lf0_smth = format_for_modelling(m_mag, m_real, m_imag, v_f0, fs,
                                                                             mag_dim=mag_dim, phase_dim=phase_dim)
...
def analysis_for_acoustic_modelling(wav_file, out_dir, fft_len=None, mag_dim=60, phase_dim=10,
    ...
    m_mag_mel_log, m_real_mel, m_imag_mel, v_lf0_smth, v_shift, fs, fft_len = analysis_compressed(wav_file,
                                                                                                  fft_len=fft_len,
                                                                                                  mag_dim=mag_dim,
                                                                                                  phase_dim=phase_dim,
                                                                                                  b_const_rate=b_const_rate,
                                                                                                  b_mag_fbank_mel=b_mag_fbank_mel,
                                                                                                  alpha_phase=b_mag_fbank_mel)
...
def analysis_compressed_type1_with_phase_comp(wav_file, fft_len=None, out_dir=None,
    ...
    if False:
        ...
        nx = 320;
        figure();
        plot(m_real_mel_orig[nx, :]);
        plot(m_real_mel[nx, :]);
        grid()
        nx = 320;
        figure();
        plot(m_real_rec_orig[nx, :]);
        plot(m_real_rec[nx, :]);
        grid()
...
def analysis_compressed_type2(wav_file, fft_len=None, out_dir=None, mag_dim=60, phase_dim=45, b_norm_mag=False,
                              const_rate_ms=-1.0):
    # Analysis:
    ...
    if False:
        ...
        lp.plot(m_real[nx, :], '.-');
        lp.plot(m_imag[nx, :], '.-');
        lp.plot(m_imag_ext[nx, :], '.-');
        lp.grid()
        nx = 0;
        lp.figure();
        lp.plot(np.diff(m_imag[nx, :]), '.-');
        lp.plot(np.diff(m_imag_ext[nx, :]), '.-');
        lp.grid()
        nx = 0;
        lp.figure();
        lp.plot(np.diff(np.diff(m_imag[nx, :])), '.-');
        lp.plot(np.diff(np.diff(m_imag_ext[nx, :])), '.-');
        lp.grid()
        nx = 0;
        lp.figure();
        lp.plot(m_real[nx, :] ** 2 + m_imag[nx, :] ** 2, '.-');
        lp.grid()
    ...
    if False:
        ...
        nx = 312;
        lp.figure();
        lp.plot(m_mag_log[nx:nx + 3, :].T);
        lp.grid()
        nx = 312;
        lp.figure();
        lp.plot(m_mag_log_norm[nx:nx + 3, :].T);
        lp.grid()
...
def synthesis_from_acoustic_modelling_old(in_feats_dir, filename_token, out_syn_dir, mag_dim, phase_dim, fs,
                                          fft_len=None, pf_type='no', magphase_type='type1', b_const_rate=False):
    '''
    ...
    if magphase_type == 'type1':
        ...
        v_syn_sig = synthesis_from_compressed_type2(m_mag_mel_log, m_real_mel, m_imag_mel, v_lf0, fs, fft_len=fft_len,
                                                    const_rate_ms=5)
...
def post_filter_merlin(m_mag_mel_log, fs, pf_coef=1.4):
    ...
    curr_cmd = '{freqt} -m {order} -a {fw} -M {co} -A 0 < {mgc} | {c2acr} -m {co} -M 0 -l {fl} > {base_r0}'. \
        format(freqt=freqt_bin, order=ncoeffs - 1, fw=alpha, co=minph_ord, mgc=temp_mcep, c2acr=c2acr_bin, fl=fft_len,
               base_r0=temp_r0)
    call(curr_cmd, shell=True)
    # --------------------------------------
    curr_cmd = '{vopr} -m -n {order} < {mgc} {weight} | {freqt} -m {order} -a {fw} -M {co} -A 0 | {c2acr} -m {co} -M 0 -l {fl} > {base_p_r0}' \
        .format(vopr=vopr_bin, order=ncoeffs - 1, mgc=temp_mcep, weight=temp_lifter, freqt=freqt_bin, fw=alpha,
                co=minph_ord,
    ...
    curr_cmd = '{vopr} -m -n {order} < {mgc} {weight} | {mc2b} -m {order} -a {fw} | {bcp} -n {order} -s 0 -e 0 > {base_b0}' \
        .format(vopr=vopr_bin, order=ncoeffs - 1, mgc=temp_mcep, weight=temp_lifter, mc2b=mc2b_bin, fw=alpha,
                bcp=bcp_bin, base_b0=temp_b0)
    call(curr_cmd, shell=True)

    # --------------------------------------
    curr_cmd = '{vopr} -d < {base_r0} {base_p_r0} | {sopr} -LN -d 2 | {vopr} -a {base_b0} > {base_p_b0}' \
        .format(vopr=vopr_bin, base_r0=temp_r0, base_p_r0=temp_p_r0, sopr=sopr_bin, base_b0=temp_b0,
                base_p_b0=temp_p_b0)
    call(curr_cmd, shell=True)
    # --------------------------------------
