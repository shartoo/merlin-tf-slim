...
def interp_unv_regions(m_data, v_voi, voi_cond='>0', interp_type='linear'):
    ...
    if interp_type == 'zeros':
        ...
        func_intrp = interpolate.interp1d(v_voiced_nx, m_data[vb_voiced, :], bounds_error=False, axis=0,
                                          fill_value=t_strt_and_end_voi_frms, kind=interp_type)
...
def sp_to_mcep(m_sp, n_coeffs=60, alpha=0.77, in_type=3, fft_len=0):
    ...
    curr_cmd = sptk_mcep_bin + " -a %1.2f -m %d -l %d -e 1.0E-8 -j 0 -f 0.0 -q %d %s > %s" % (
...
def sp_mel_warp_fbank_2d(m_mag, n_melbands, alpha=0.77):
    ...
    for nxb in range(v_td_span.size):
