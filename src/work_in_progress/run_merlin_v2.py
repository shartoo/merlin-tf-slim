...
def train_DNN(train_xy_file_list, valid_xy_file_list, \
    ...
    if model_type == 'DNN':
        dnn_model = DeepRecurrentNetwork(n_in=n_ins, hidden_layer_size=hidden_layer_size, n_out=n_outs,
                                         ...
                                         output_type=cfg.output_layer_type, network_type=cfg.network_type,
                                         dropout_rate=dropout_rate)
        if cfg.network_type == 'S2S':
            train_fn, valid_fn = dnn_model.build_finetune_functions_S2SPF(
                (train_set_x, train_set_y, train_set_d, train_set_f),
                (valid_set_x, valid_set_y, valid_set_d, valid_set_f))
        else:
...
def main_function(cfg):
    ...
    if cfg.NORMLAB and (cfg.label_style == 'HTS'):
        ...
        if cfg.network_type == "S2S":
            ...
            label_normaliser.prepare_dur_data(in_label_align_file_list, seq_dur_file_list, feature_type="numerical",
                                              unit_size="phoneme")
    ...
    nnets_file_name = '%s/%s_%s_%d_%s_%d.%d.train.%d.%f.rnn.model' \
                      % (model_dir, cfg.combined_model_name, cfg.combined_feature_name, int(cfg.multistream_switch),
                         ...
                         cfg.hyper_params['learning_rate'])
    ...
    if cfg.TRAINDNN:
        ...
        try:
            train_DNN(train_xy_file_list=(train_x_file_list, train_y_file_list), \
                      ...
                      seq_dur_file_list=seq_dur_file_list)
    ...
    if cfg.GENBNFEA:
        ...
        test_d_file_list = seq_dur_file_list[
    ...
    if cfg.DNNGEN:
        ...
        if cfg.AcousticModel:
            ...
            generator.acoustic_decomposition(gen_file_list, cfg.cmp_dim, cfg.out_dimension_dict,
                                             cfg.file_extension_dict, var_file_dict, do_MLPG=cfg.do_MLPG)
