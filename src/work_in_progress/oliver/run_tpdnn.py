...
def train_DNN(train_xy_file_list, valid_xy_file_list, \
    ...
    if model_type == 'DNN':
        ...
        dnn_model = TokenProjectionDNN(numpy_rng=numpy_rng, n_ins=n_ins, n_outs=n_outs,
                                       ...
                                       expand_by_minibatch=expand_by_minibatch,
    ...
    if do_pretraining and model_type == 'SDAE':
        ...
        for i in range(dnn_model.n_layers):
            for epoch in range(pretraining_epochs):
                ...
                logger.info('Pre-training layer %i, epoch %d, cost %s, time spent%.2f' % (
                i + 1, epoch + 1, numpy.mean(pretrain_loss), (sub_end_time - sub_start_time)))
                train_data_reader.reset()
    ...
    while (epoch < training_epochs):
        ...
        logger.info('BASIC epoch %i, validation error %f, train error %f  time spent %.2f' % (
...
def train_DNN_with_projections(train_xy_file_list, valid_xy_file_list, \
    ...
    while (epoch < all_epochs):
        ...
        logger.info('TOKEN epoch %i, validation error %f, train error %f  time spent %.2f' % (
        epoch, this_validation_loss, this_train_valid_loss, (sub_end_time - sub_start_time)))
...
def infer_projections(train_xy_file_list, valid_xy_file_list, \
    ...
    if cfg.hyper_params['model_type'] == 'TPDNN':
        os.system('python %s %s' % ('/afs/inf.ed.ac.uk/user/o/owatts/scripts_NEW/plot_weights_multiple_phases.py',
                                    cfg.projection_weights_output_dir))
...
def main_function(cfg):
    ...
    if cfg.NORMCMP:
        ...
        if cfg.output_feature_normalisation == 'MVN':
            ...
            global_mean_vector = normaliser.compute_mean(nn_cmp_file_list[0:cfg.train_file_number])
            global_std_vector = normaliser.compute_std(nn_cmp_file_list[0:cfg.train_file_number], global_mean_vector)

            normaliser.find_min_max_values(nn_cmp_file_list[0:cfg.train_file_number])
            normaliser.normalise_data(nn_cmp_file_list, nn_cmp_norm_file_list)
    ...
    if cfg.DNNGEN:
        ...
        dnn_generation(test_x_file_list, nnets_file_name, lab_dim, cfg.cmp_dim, gen_file_list_NOTOKENS, cfg=cfg,
                       use_word_projections=False)

        ## With word embeddings:
        gen_file_list_TOKENS = prepare_file_path_list(gen_file_id_list, gen_dir_TOKENS, cfg.cmp_ext)
        dnn_generation(test_x_file_list, nnets_file_name, lab_dim, cfg.cmp_dim, gen_file_list_TOKENS, cfg=cfg,
                       use_word_projections=True)
        ...
        for gen_file_list in [gen_file_list_NOTOKENS, gen_file_list_TOKENS]:
            ...
            if cfg.output_feature_normalisation == 'MVN':
                ...
                denormaliser = math_statis.Statis(feature_dimension=cfg.cmp_dim,
    ...
    if cfg.CALMCD:
        ...
        if 'lf0' in cfg.in_dimension_dict:
            ...
            valid_f0_mse, valid_vuv_error = calculator.compute_distortion(valid_file_id_list, ref_data_dir, gen_dir,
                                                                          cfg.lf0_ext, cfg.lf0_dim)
            test_f0_mse, test_vuv_error = calculator.compute_distortion(test_file_id_list, ref_data_dir, gen_dir,
                                                                        cfg.lf0_ext, cfg.lf0_dim)
        ...
        if 0:  # to calculate distortion of HMM baseline
            ...
            spectral_distortion = calculator.compute_distortion(valid_file_id_list, ref_data_dir,
                                                                hmm_gen_no_silence_dir, cfg.mgc_ext, cfg.mgc_dim)
            bap_mse = calculator.compute_distortion(valid_file_id_list, ref_data_dir, hmm_gen_no_silence_dir,
                                                    cfg.bap_ext, cfg.bap_dim)
            f0_mse, vuv_error = calculator.compute_distortion(valid_file_id_list, ref_data_dir, hmm_gen_no_silence_dir,
                                                              cfg.lf0_ext, cfg.lf0_dim)

            spectral_distortion *= (10 / numpy.log(10)) * numpy.sqrt(2.0)
            bap_mse = bap_mse / 10.0

            logger.info('Develop: HMM -- MCD: %.3f dB; BAP: %.3f dB; F0: %.3f Hz; VUV: %.3f%%' % (
            spectral_distortion, bap_mse, f0_mse, vuv_error * 100.))

            spectral_distortion = calculator.compute_distortion(test_file_id_list, ref_data_dir, hmm_gen_no_silence_dir,
                                                                cfg.mgc_ext, cfg.mgc_dim)
            bap_mse = calculator.compute_distortion(test_file_id_list, ref_data_dir, hmm_gen_no_silence_dir,
                                                    cfg.bap_ext, cfg.bap_dim)
            f0_mse, vuv_error = calculator.compute_distortion(test_file_id_list, ref_data_dir, hmm_gen_no_silence_dir,
                                                              cfg.lf0_ext, cfg.lf0_dim)

            spectral_distortion *= (10 / numpy.log(10)) * numpy.sqrt(2.0)
            bap_mse = bap_mse / 10.0

            logger.info('Test   : HMM -- MCD: %.3f dB; BAP: %.3f dB; F0: %.3f Hz; VUV: %.3f%%' % (
            spectral_distortion, bap_mse, f0_mse, vuv_error * 100.))
