        ...
        while (not train_data_reader.is_finish()):
            ...
            for minibatch_index in range(n_train_batches):
                ...
                if seen_frames >= 720000:  ## Hardcoded checking intervals and framerate: 720000 frames per hour at 5ms frame rate
                    ...
                    print('Mean validation loss: %s, std over minibatches: %s' % (
                    this_validation_loss, this_validation_loss_std))
                    ...
                    logger.info('epoch %i, validation error %f (std: %f), train error %f (std: %f)  time spent %.2f' % (
                    epoch, this_validation_loss, this_validation_loss_std, this_train_valid_loss,
                    this_train_valid_loss_std, (sub_end_time - sub_start_time)))
                    if plot:
                        plotlogger.add_plot_point('training convergence', 'validation set',
                                                  (hours_seen, this_validation_loss))
                        plotlogger.add_plot_point('training convergence', 'training set',
                                                  (hours_seen, this_train_valid_loss))
                        plotlogger.save_plot('training convergence', title='Progress of training and validation error',
                                             xlabel='hours of data seen', ylabel='error')
