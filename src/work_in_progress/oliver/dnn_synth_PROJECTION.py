...
def within_circle(dots, radius=1.0):
    ...
    within_circle = (standardised_dots[:, 0] * standardised_dots[:, 0]) + (
    standardised_dots[:, 1] * standardised_dots[:, 1]) <= 1.0
...
def add_projection_indices(uttlist, token_xpath, attrib_name, outdir):
    ...
    for uttfile in uttlist:
        ...
        root_node.attrib[
            attrib_name] = '0'  ## 0 is the defualt 'n/a' value -- *some* ancestor of all nodes will have the relevant attibute to fall back on
...
def main_function(cfg, in_dir, out_dir, token_xpath, index_attrib_name, synth_mode, cmp_dir, projection_end):
    ...
    indexed_utt_dir = os.path.join(out_dir,
                                   'utt')  ## place to put test utts with tokens labelled with projection indices
    ...
    if synth_mode == 'single_sentence_demo':
        synth_utts_input = add_projection_indices_with_replicates(synth_utts_input, token_xpath, index_attrib_name,
                                                                  indexed_utt_dir, 100)
    else:
    ...
    for label_style, label_style_required in label_composer.label_styles.items():
        if label_style_required:
            ...
            if label_style == 'xpath':
                in_label_align_file_list['xpath'] = prepare_file_path_list(file_id_list, indexed_utt_dir, cfg.utt_ext,
                                                                           False)
    ...
    if synth_mode == 'inferred':
        ...
        for feature_name in list(cfg.in_dir_dict.keys()):
            ...
            in_file_list_dict[feature_name] = prepare_file_path_list(file_id_list, in_direc,
    ...
    dnn_generation_PROJECTION(nn_label_norm_file_list, nnets_file_name, lab_dim, cfg.cmp_dim, gen_file_list, cfg=cfg,
                              synth_mode=synth_mode, projection_end=projection_end,
                              projection_weights_to_use=inferred_weights, save_weights_to_file=weights_outfile)
    ...
    simple_scale_variance(gen_dir, scaled_dir, var_file_dict, cfg.out_dimension_dict, file_id_list,
                          gv_weight=0.5)  ## gv_weight hardcoded
...
if __name__ == '__main__':
    ...
    assert synth_mode in ['constant', 'sampled_training', 'inferred', 'uniform', 'single_sentence_demo',
                          'uniform_sampled_within_std_1', 'uniform_sampled_within_std_2',
                          'uniform_sampled_within_std_3']
