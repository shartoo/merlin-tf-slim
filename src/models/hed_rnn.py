...
from layers.gating import SimplifiedLstm, SimplifiedLstmDecoder, BidirectionSLstm, VanillaLstm, VanillaLstmDecoder, \
    BidirectionLstm, VanillaRNN, VanillaRNNDecoder, SimplifiedGRU, GatedRecurrentUnit
from layers.layers import GeneralLayer, LinearLayer
...
from models.seq2seq import DistributedSequenceEncoder
...
class DeepEncoderDecoderNetwork(object):
    ...
    def __init__(self, n_in, hidden_layer_size, n_out, L1_reg, L2_reg, hidden_layer_type, output_type='LINEAR',
                 network_type='S2S', ed_type='HED', dropout_rate=0.0, optimizer='sgd', MLU_div_lengths=[],
                 loss_function='MMSE', rnn_batch_training=False):
        ...
        for i in range(self.n_layers):
            ...
            if hidden_layer_type[i] in self.list_of_activations:
                ...
                hidden_layer = SimplifiedLstmDecoder(rng, layer_input, input_size, hidden_layer_size[i], self.n_out,
                                                     p=self.dropout_rate, training=self.is_train,
                ...
                hidden_layer = VanillaLstmDecoder(rng, layer_input, input_size, hidden_layer_size[i], self.n_out,
                                                  p=self.dropout_rate, training=self.is_train,
                                                  rnn_batch_training=self.rnn_batch_training)
                ...
                hidden_layer = VanillaRNNDecoder(rng, layer_input, input_size, hidden_layer_size[i], self.n_out,
                                                 p=self.dropout_rate, training=self.is_train,
                                                 rnn_batch_training=self.rnn_batch_training)
    ...
    def parameter_prediction(self, test_set_x):  # , batch_size
        ...
        test_out = theano.function([], self.final_layer.output,
                                   givens={self.x: test_set_x[0:n_test_set_x], self.is_train: np.cast['int32'](0)},
    ...
    def parameter_prediction_S2S(self, test_set_x, test_set_d):
        ...
        test_out = theano.function([], self.final_layer.output,
                                   givens={self.x: test_set_x[0:n_test_set_x], self.d: test_set_d[0:n_test_set_x],
                                           self.is_train: np.cast['int32'](0)}, on_unused_input='ignore')
    ...
    def parameter_prediction_S2SPF(self, test_set_x, test_set_d, test_set_f):
        ...
        test_out = theano.function([], self.final_layer.output,
                                   givens={self.x: test_set_x[0:n_test_set_x], self.d: test_set_d, self.f: test_set_f,
                                           self.is_train: np.cast['int32'](0)}, on_unused_input='ignore')
