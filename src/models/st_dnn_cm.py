...
class SequentialDNN(object):
    def __init__(self, numpy_rng, n_ins=100,
        ...
        for i in range(self.n_layers):
            ...
            W_value = gnp.garray(
                numpy_rng.normal(0.0, 1.0 / np.sqrt(input_size), size=(input_size, hidden_layer_sizes[i])))
    ...
    def backpropagation(self, train_set_y, mean_matrix, std_matrix):
        ...
        for sub_dim in sub_dim_list:
            wuw_mat, wu_mat = self.pre_wuw_wu(frame_number, sub_dim,
        ...
        propagate_error = gnp.dot(observation_error,
                                  self.W_params[self.n_layers].T)  # final layer is linear output, gradient is one
    ...
    def feedforward(self, train_set_x):
        ...
        # output layers
        self.final_layer_output = gnp.dot(self.activations[self.n_layers], self.W_params[self.n_layers]) + \
                                  self.b_params[self.n_layers]
    ...
    def pre_wuw_wu(self, frame_number, static_dimension, var_base):
        ...
        for i in range(static_dimension):
            ...
            wuw_mat[frame_number * i:frame_number * (i + 1), frame_number * i:frame_number * (i + 1)] = gnp.garray(
                temp_wuw[:])
            wu_mat[frame_number * i:frame_number * (i + 1), frame_number * i:frame_number * (i + 3)] = gnp.garray(
                temp_wu[:])
