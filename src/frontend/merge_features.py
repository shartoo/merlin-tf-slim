    ...
    def merge_data(self, binary_label_file_list, new_feat_file_list, out_feat_file_list):
        ...
        for i in range(utt_number):
            ...
            if (lab_frame_number - feat_frame_number) > 5:
                ...
                self.logger.critical("the number of frames in label and new features are different: %d vs %d (%s)" % (
            ...
            merged_features[0:feat_frame_number, self.lab_dim:self.lab_dim + self.feat_dim] = new_features[
                                                                                              0:lab_frame_number, ]

            io_funcs.array_to_binary_file(merged_features, out_feat_file_name)
            self.logger.debug('merged new feature %s of %d frames with %d label features' % (
